const data = [
  {
    id: 1,
    title: "Culture Exchanges & Programs",
    cards: [
      {
        image: "row_1_1",
        label: "IYLEP",
        description: "Iraqi Young Leaders Exchange...",
        link: "/CultureExchanges/8"
      },
      {
        image: "row_1_2",
        label: "SUSI",
        description: "The Study of the United States...",
        link: "/CultureExchanges/10"
      },
      {
        image: "row_1_3",
        label: "SLP",
        description: "The Student Leaders Program...",
        link: "/CultureExchanges/9"
      },
      {
        image: "row_1_4",
        label: "CEE",
        description: "The Community Engagement Exc...",
        link: "/CultureExchanges/11"
      },
      {
        image: "row_1_10",
        label: "Istanbul cultural Trip",
        description: "You are warmly invited to partake at Korean...",
        link: "/CultureExchanges/2"
      },
      {
        image: "row_1_11",
        label: "Travel to Korea",
        description: "The hottest tracks in India. Cover: Taylor...",
        link: "/CultureExchanges/3"
      },
      {
        image: "row_1_12",
        label: "Travel to Japan",
        description: "Get to know about Japanese Culture...",
        link: "/CultureExchanges/1"
      },
      {
        image: "row_1_5",
        label: "",
        description: "",
        link: "/CultureExchanges"
      },

    ],
  },
  {
    id: 6,
    title: "US Embassy Programs",
    cards: [
      {
        image: "row_1_1",
        label: "IYLEP",
        description: "Iraqi Young Leaders Exchange...",
        link: "/CultureExchanges/8"
      },
      {
        image: "row_1_2",
        label: "SUSI",
        description: "The Study of the United States...",
        link: "/CultureExchanges/10"
      },
      {
        image: "row_1_10",
        label: "SUSI",
        description: "It is a year-long Asian cultural exch...",
        link: "/CultureExchanges/5"
      },
      {
        image: "row_1_3",
        label: "SLP",
        description: "The Student Leaders Program...",
        link: "/CultureExchanges/9"
      },
      {
        image: "row_1_4",
        label: "CEE",
        description: "The Community Engagement Exc...",
        link: "/CultureExchanges/11"
      },


    ],
  },
  {
    id: 2,
    title: "Virtual Experience Programs",
    cards: [
      {
        image: "row_2_1",
        label: "On-Premise Sales",
        description: "Learn from the world’s top sales specialists...",
        link: "/VirtualPrograms/1"
      },
      {
        image: "row_2_2",
        label: "Cybersecurity",
        description: "Explore a day in the life of a Security...",
        link: "/VirtualPrograms/2"
      },
      {
        image: "row_2_3",
        label: "Advanced Software Engineering",
        description: "Take the opportunity to get a true...",
        link: "/VirtualPrograms/3"
      },
      {
        image: "row_1_5",
        label: "",
        description: "",
        link: "/VirtualPrograms"
      },
    ],
  },
  {
    id: 3,
    title: "Governmental",
    cards: [
      {
        image: "row_3_1",
        label: "National Youth Group",
        description: "Ministry of Youth and Sports...",
        link: "/Governmental/1"
      },
      {
        image: "row_1_5",
        label: "",
        description: "",
        link: "/Governmental"
      },
    ],
  },
  {
    id: 4,
    title: "Internships & Jobs",
    cards: [
      {
        image: "row_4_1",
        label: "I.S.M.S. Co. Ltd",
        description: "Chill instrumental beats. Artwork: Gize...",
        link: "/internships/13"
      },
      {
        image: "row_4_2",
        label: "UNICEF Internship",
        description: "Do you want to make a difference...",
        link: "/internships/6"
      },
      {
        image: "row_4_3",
        label: "OIST Internship",
        description: "OIST Internship in Japan 2024 is...",
        link: "/internships/7"
      },
      {
        image: "row_4_4",
        label: "Rwanga Vacancies",
        description: "Rwanga Foras is a project by Rwanga...",
        link: "/internships/8"
      },
      {
        image: "row_4_5",
        label: "Chevening Fellowships 2024",
        description: "Chevening Fellowships 2024 is one of the...",
        link: "/internships/9"
      },
      {
        image: "row_4_6",
        label: "OHCHR Indigenous Fellowship ",
        description: "OHCHR Indigenous Fellowship Pr...",
        link: "/internships/10"
      },
      {
        image: "row_4_7",
        label: "OIST Internship in Japan 2024",
        description: "OIST Internship in Japan 2024 is a uniq...",
        link: "/internships/11"
      },
      {
        image: "row_1_5",
        label: "",
        description: "",
        link: "/internships"
      },
    ],
  },
  {
    id: 5,
    title: "Training Abroad",
    cards: [
      {
        image: "row_5_1",
        label: "Workshop 2023 in Amsterdam",
        description: "We invite young leaders who wish...",
        link: "/Training/1"
      },
      {
        image: "row_5_2",
        label: "MENA Scholarship Programme",
        description: "The MENA Scholarship Programme (MSP)...",
        link: "/Training/2"
      },
      {
        image: "row_5_3",
        label: "Intern Program Cycle",
        description: "Do you want to help our clients...",
        link: "/Training/3"
      },
      {
        image: "row_5_4",
        label: "CERN Administrative Student ",
        description: "CERN Administrative Student Pro...",
        link: "/Training/4"
      },
      {
        image: "row_5_5",
        label: "Knight Hennessy Scholarship",
        description: "Knight Hennessy Scholarship Progra...",
        link: "/Training/5"
      },
      {
        image: "row_5_6",
        label: "Internship in Switzerland",
        description: "At CERN, the European Organization...",
        link: "/Training/6"
      },
      {
        image: "row_1_5",
        label: "",
        description: "",
        link: "/Training"
      },
    ],
  },
  {
    id: 5,
    title: "Others",
    cards: [
      {
        image: "row_6_3",
        label: "TEDxNishtiman 2024",
        description: "Feeling like everything just plain sucks?We'v...",
        link: "/others/1"
      },
      {
        image: "row_6_4",
        label: "British Council IELTS",
        description: "British Council IELTS Preparation Program 2023",
        link: "/others/2"
      },
      {
        image: "row_6_5",
        label: "ADBI E-learning program",
        description: "ADBI E-learning program is an...",
        link: "/others/3"
      },
      {
        image: "row_6_6",
        label: "Open University Online",
        description: "Make your time worthwhile by enrolli...",
        link: "/others/4"
      },
            {
        image: "row_6_7",
        label: "UNICEF Online Courses",
        description: "UNICEF Online Courses with Free...",
        link: "/others/5"
      },
      {
        image: "row_1_5",
        label: "",
        description: "",
        link: "/others"
      },
    ],
  },
];

export { data };
